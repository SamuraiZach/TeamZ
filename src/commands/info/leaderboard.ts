...
export const aliases = ['rank', 'leader'***REMOVED***
...
export async function run(msg: Message, [page]: [number]): Promise<Message> {
	...
	for (const user of displUsers) {
		...
		roundedRect(ctx, cursor.x, cursor.y, Leaderboard.width, Leaderboard.userPillHeight, 10);
...
export function argParser(_msg: Message, input: string): Array<number | null> {
	return [parseInt(input) > 1 ? parseInt(input) : 1***REMOVED***
}
